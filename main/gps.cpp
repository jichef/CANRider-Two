...
static bool modemLock(const char* tag, uint32_t ms) {
  if (modemMutex && xSemaphoreTake(modemMutex, pdMS_TO_TICKS(ms)) == pdTRUE) {
    return true;
  }
  return false;
}

static void modemUnlock(const char* tag) {
  if (modemMutex) {
...
static bool lockWithArbitration(const char* tag, uint32_t total_ms, uint32_t slice_ms) {
  // Simplificado: ya no hay arbitraje con Telegram
  return modemLock(tag, total_ms);
}
...
bool obtenerGPS(String &respuesta,
  ...
  logMsg(LOG_INFO, "GPS", "Iniciando búsqueda de FIX...");
  ...
  while (!s_abortReq && (millis() - start < timeout_ms) && !got) {
    ...
    if (target_idx != last_target_idx) {
      logMsg(LOG_INFO, "GPS", String("Nuevo objetivo: ") + kTiers[target_idx].name);
    ...
    if (fixNow) {
      ...
      if (!have_best ||
        ...
        logMsg(LOG_INFO, "GPS", String("Mejor FIX visto: ") + best_lat + "," + best_lon + " (HDOP=" + String(best_hdop,1) + ")");
      }
...
bool gps_get_position(float &lat, float &lng, float &speed, float &course) {
  if (!s_gpsActive) return false;
  
  if (!lockWithArbitration("GPS_GET_POS", 800, 300)) return false;
  ...
  if (line.startsWith("+CGNSINF:")) {
      ...
      // Formato: <GNSS run status>,<Fix status>,<UTC date & time>,<Latitude>,<Longitude>,<MSL Altitude>,<Speed Over Ground>,<Course Over Ground>,...
      int ti = 0, start = 0;
      String tok[10];
      for (int i = 0; i < payload.length() && ti < 10; ++i) {
          if (payload[i] == ',') {
      ...
      if (ti >= 8) {
          if (tok[1] == "1") { // Fix status OK
              lat = tok[3].toFloat();
              lng = tok[4].toFloat();
              speed = tok[6].toFloat();
              course = tok[7].toFloat();
              success = true;
          }
      }
  }
  
  modem.waitResponse(200, "OK");
  modemUnlock("GPS_GET_POS");
  return success;
}
