...
void checkSoC() {
  ...
  if (!apagado_ya && soc_critico_confirmado()) {
    logMsg(LOG_WARN, "BATERIA", "🔌 SoC crítico detectado (0%).");
    apagado_ya = true;
    return;
  }
  ...
  if (eff < SOC_ALARMA && !alerta_enviada_interna) {
    logMsg(LOG_WARN, "BATERIA", "⚠️ Batería baja: " + String(eff) + "%");
...
void checkSoCOnBoot() {
  ...
  if (soc_critico_confirmado()) {
    Serial.println("⚠️ SoC crítico confirmado en arranque");
    // Podrías poner aquí un deep sleep si quieres forzar el apagado
  }
}
